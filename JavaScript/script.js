...
function renderTabs() {
    ...
    for (const name in lists) {
        ...
        btn.onclick = () => { saveLists(); currentList = name; todos = lists[name]; listTitle.textContent = name; renderTabs(); render(); };
        ...
        del.onclick = e => {
            ...
            listTitle.textContent = currentList;
            saveLists(); renderTabs(); render();
        };
        wrapper.appendChild(btn); wrapper.appendChild(del); listTabs.appendChild(wrapper);
    }
}
...
addListBtn.addEventListener("click", () => {
    ...
    if (lists[trimmed]) { alert("Existiert bereits!"); return; }
    lists[trimmed] = []; currentList = trimmed; todos = lists[trimmed]; listTitle.textContent = trimmed;
    saveLists(); renderTabs(); render();
});
